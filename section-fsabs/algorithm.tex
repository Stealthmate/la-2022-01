...
\begin{algorithm}
  ...
  \Else {
    $(t_1, d_1) \gets$ \FToEnd{$m$, $U$, $A$, $r$}\; \label{alg:line:toNextW}
  }
  ...
  \If{$\forall i > r \pause A^{(1)}_i = 1$}{ \label{alg:line:check-only-riders}
    $S^{(1)} \gets$ \FMoveRiders{$m$, $U$, $A^{(1)}$}\;
  }
  \ElseIf{$d < 1$}{ \label{alg:line:check-not-arrived-yet}
    $A^{(2)} \gets \frac{A^{(1)} - d}{1 - d}$\;
    $S^{(2)} \gets$ \FSolveFSABS{$m$, $U$, $A^{(2)}$}\;
    $S^{(1)} \gets$ \FScaleSchedule{$S^{(2)}$, $1 - d$}\;
  }
...
\item 人 $i$ がグループの人 ($r < i < m - f$) ならば，グループ全体のスケジュール $(M, X)$ を BS の解として求め， \lemref{bs-scalable} を用いて距離 $\frac{t}{T(m - f - r, U_{:b-r})}$ に合わせた上で， $(M, X)$ に相当する自由スケジュール $S^\prime$ を組み立て，人 $i$ の自由スケジュールを $S^\prime_i$ とする．
...
次に移動後の状態を改めて分析した上で次の挙動を決める．もしグループの移動距離が全区間ならば，前方の歩行者とグループが必ず到着点に到達したと解釈できる．その祭，$i > r$ に対し $A^{(1)}_i$ の値は 1 となる．しかし，もしライダーがいる場合にはライダーがまだ到着点に到達していない可能性があり，ライダーのスケジュールにそれぞれが到達するまでの分を追加追加しなければならない．この場合に対応するのが \lineref{check-only-riders} の条件分岐である．

...
\begin{lemma}\label{lemma:horizontal-composition}
  任意の入力 $(m, U, A^{(1)})$ 及び $(m, U, A^{(2)})$ それぞれに対し自由スケジュール $S^{(1)}$ 及び $S^{(2)}$ を考える． $S^{(1)}$ に対し \defref{fsabs-feasible-schedule} の条件 1 以外が成り立つとし， $S^{(2)}$ が実行可能である (条件が全て成り立つ) とする．もしとある定数 $d$  及び $S^{(1)}$ の全ての行 $S^{(1)}_i$ に対し
  \begin{enumerate}
  \item $\displaystyle A^{(1)}_i + L(S^{(1)}_i) = \frac{1}{1-d}(A^{(2)}_i - d)$，
  \item $A^{(2)}_i < 0$ ならば $\beta^{(2)}_{i,1} = \beta^{(1)}_{i,n_i^{(1)}}$ \label{requirement:same-bikes}
  \end{enumerate}
  が成り立つなら，以下で生成される $S^{(3)} = S^{(1)} + S^{(2)}$ は $(m, U, A^{(1)})$ に対する実行可能解である．
  \begin{equation}
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
ライダーの分と部分グループの分に対し独立に \defref{fsas-feasible-schedule} の条件が成り立つので， \propref{vertical-composition} より $S^{(out)}$ は実行可能であり．到着時間が $\tau^\prime(S^{(out)}) = \bar\tau(m, U) = u_b$ となり， \lemref{fsabs-lower-bound-bike} が満たされる．
...
% T(m, U, a) = 1 - 1/m (sum bikes) - a/m
% ub > T(m - 1, U)
% ub < T(m, U1)
% ub < 1 - 1/m(U1)
% ub < 1 - 1/m(sum 1 - u_i)_1
% mub < m - sum(1 - u_i)_1
% mub + (1 - ub) < m - sum(1 - u_i)_all
% (m - 1)ub + 1 < m - sum(1 - u_i)_all
% (m - 1)ub < m - 1 - sum(1 - u_i)_all
% ub < 1 - 1/(m - 1)(sum U) <- contradiction?
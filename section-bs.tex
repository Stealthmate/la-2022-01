...
\begin{lemma} (\textcite{czyzowicz})\label{lemma:bs-subgroup}
  $(m, U)$ を BS の入力とする． $u_b \geq T(m, U)$ ならば次を満たす $k$ が存在する．ただし， $U_k = [u_1, u_2, \ldots, u_{k}]$ 及び $m_k = m - b + k$ とする．
  \begin{equation}
    u_k \leq T(m_k, U_k) \leq u_b
  \end{equation}
\end{lemma}
...
\begin{claim}\label{claim:bs-scalable}
  BS の入力 $(m, U)$ に対し $(M, X)$ を実行可能解とする．任意の区間 $[0, a]$ に対し BS を定義したとき，同じ入力に対し $(M, aX)$ が実行可能解となり， $\tau(M, aX) = a\tau(M, X)$．
\end{claim}